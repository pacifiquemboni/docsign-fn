...
import type { DocumentField, ResizeFieldPayload } from '../types';
import type { APIListResponse } from '@/types/api';

export function useResizeField(documentId: string) {
  const qc = useQueryClient();
  const key = fieldsQueryKey(documentId);

  return useMutation({
    mutationFn: ({ fieldId, payload }: { fieldId: string; payload: ResizeFieldPayload }) =>
      fieldApi.resize(fieldId, payload),
